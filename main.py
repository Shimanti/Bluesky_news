...
# --- URL Shortener has been REMOVED. We don't need it! ---

# --- MODIFIED Part: A simpler, more direct prompt for Gemini ---
def create_bluesky_text(title):
    """Uses Gemini to create a summary to appear above a link card."""
    genai.configure(api_key=os.environ.get("GEMINI_API_KEY"))
    model = genai.GenerativeModel('gemini-1.5-flash')
    
    # This prompt is now simple. We have the full 300 characters to work with.
    prompt = f"""
...
# --- Part C: Get Last Post (Unchanged, but now more important) ---
def get_last_posted_link(client, handle):
    ...
    try:
        ...
        # Check specifically for the link card embed
...
def post_to_bluesky(client, text_body, original_article_url):
    ...
    try:
        # The library automatically detects the URL and creates the card!
        client.send_post(text=f"{text_body} {original_article_url}")
        print("Successfully posted to BlueSky with a link card!")
    except Exception as e:
        print(f"Error posting to BlueSky: {e}")

# --- MODIFIED Part: Main Execution (Much cleaner) ---
        ...
        if not (title and link):
            ...
            if link == last_link:
                ...
                if post_text:
                    ...
                    # We NO LONGER shorten the link. We just post the text and the ORIGINAL link.
                    # BlueSky's API will handle the rest.
